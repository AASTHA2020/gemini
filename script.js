document.addEventListener("DOMContentLoaded", function() {
    ...
    const geminBoxContainer = document.getElementById("geminBoxContainer");
    ...
    async function run() {
        ...
        divUser.innerHTML = `
            <img class="w-[30px] h-[30px] rounded-full hover:cursor-pointer" src="profile-pic(8).png" alt="" sizes="" srcset="">
            <div class="flex justify-center items-center text-white bg-[rgb(30,31,32)] px-[20px] py-[10px] rounded-lg">${searchBox.value}</div>
        `;
        displayContainer.appendChild(divUser);
        let divBotheight = displayContainer.clientHeight - divUser.clientHeight;
        ...
        `;
        displayContainer.appendChild(divBot);
        divBotheight = displayContainer.clientHeight - divBot.clientHeight;
        window.scrollTo(0, divBotheight);
    }
    ...
    // Add event listener for image input if needed
    iconInputImageBtn.addEventListener("click", () => {
        inputImage.click();
    });

    // Additional functionality can be added as per your requirements
});
